...
const PromptResponsePicker = ({ players, roundState, onSubmit }: Props) => {
  ...
  if (recordedResponse) {
    ...
    return (
      <SafeAreaView className="p-5 items-center">
        <Text className="text-center text-base font-alfa">
  ...
  if (!selectedId) {
    return (
      <SafeAreaView className="flex-1 p-5">
        <Text className="dark:text-light text-dark mb-5 font-alfa">Who's answering?</Text>
        {players.map((p) => (
          <Pressable
          ...
          >
            <Text className="text-center font-alfa">{p.displayName}</Text>
          </Pressable>
        ))}
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView>
      <Text className="text-[#888] text-center text-[14px] mb-5 font-alfa">
      ...
      <View className="flex flex-row gap-2.5 w-full">
        ...
        >
          <Text className="text-center text-white font-alfa">Answered</Text>
        </Pressable>
        <Pressable
          className="flex-1 py-3 rounded-lg items-center bg-[#c0392b]"
          onPress={() => { onSubmit(selectedId, false); setSelectedId(null); }}
        >
          <Text className="text-center text-white font-alfa">Drank</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
